...
/**
 * Heals nearby damaged Xoonglins (and optionally the leader) by spending items. If a
 * required structure is set, the healer works from it, treating patients within a radius
 * of the structure and taking supplies from its containers; otherwise it works from its
 * home. Supplies are carried in the healer's inventory and restocked from the base.
 */
public class HealerJob extends Job {
    ...
    public boolean hasDose(XoonglinEntity healer) {
        SimpleContainer inventory = healer.getInventory();
    ...
    /** Removes one heal's worth of items from the healer's inventory. */
    public void consumeDose(XoonglinEntity healer) {
        SimpleContainer inventory = healer.getInventory();
        for (ItemQuantity item : items) {
    ...
    public void eraseMemories(XoonglinEntity xoonglin) {
        xoonglin.getBrain().eraseMemory(CftMemoryModuleType.MUST_HEAL.get());
    ...
    public Codec<? extends Job> jobType() {
        return CftRegistry.HEALER_JOB.get();
    }
}
