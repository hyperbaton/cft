...
public class CheckOnXoonglinPacket {
    ...
    public CheckOnXoonglinPacket(String socialClass, double happiness, Map<String, Double> needsSatisfaction) {
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        ...
        if (player != null) {
            player.sendSystemMessage(Component.literal("Xoonglin class: ").append(Component.translatable(this.socialClass)));
            player.sendSystemMessage(Component.literal("Xoonglin happiness: " + String.format("%.2f", this.happiness)));
            player.sendSystemMessage(Component.literal("Xoonglin needs: "));
            for (Map.Entry<String, Double> entry : needsSatisfaction.entrySet()) {
