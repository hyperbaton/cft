...
public class RandomStrollBehavior  extends Behavior<XoonglinEntity> {
    ...
    @Override
    protected void start(@NotNull ServerLevel pLevel, XoonglinEntity xoonglin, long pGameTime) {
        xoonglin.getNavigation().moveTo(
                xoonglin.getNavigation().createPath(createRandomPath(pLevel, xoonglin.blockPosition(), 1, 10.0),
                        1),
                0.8);
    }
    ...
    protected boolean canStillUse(ServerLevel level, XoonglinEntity xoonglin, long pGameTime) {
        return xoonglin.getNavigation().isInProgress();
    }
